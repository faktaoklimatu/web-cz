(function () {
  ...
  //   measure's own capex_sensitivity (so the toggle matches the sensitivity analysis).
  const state = {
    carbonPrice:           70,
    discountRate:           3,
    fuelScenario:          'CP',
  ...
  function computeRow(measure) {
    try {
      ...
      // Band (low/high) is always supplied by computeBand() at the call site; here we
      // only carry the point value.
      return {
        npv:         { value: result.npv },
  ...
  function renderSummaryChart(container) {
    ...
    svg.select('.npv-hdr').attr('x', SUMMARY_LABEL_W + chartW / 2).attr('y', 16)
      .text('ROZDÍL V NET PRESENT VALUE OPROTI EMISNĚ NÁROČNĚJŠÍ VARIANTĚ (Kč)');
  ...
  // Visual style: Inter typography, slate/grey palette, energy-class badges,
  // block/circle pictographs for CO₂ & fossil-import, diverging NPV dot colour.
  const ROW_H   = 72;
  const LABEL_W = 210;
  const CO2_W   = 120;
  const FUEL_W  = 160;
  ...
  // Leading illustration icon per measure (shared SVG icon set).
  const CB_ICON_BASE   = '/assets-local/figures/2026-analyza-opatreni-domacnosti';
  const MEASURE_ICONS  = {
  ...
  function npvDotColor(npv) { return npv >= 0 ? COLOR_FAVORABLE : COLOR_COSTLY; }
  ...
  function contextPrefix(prefix, measureName) {
    if (FUEL_IN_CONTEXT.includes(measureName)) {
      return prefix.replace(/\s+uhlí\s*$/i, ' (uhelný kotel)').replace(/\s+plyn\s*$/i, ' (plynový kotel)');
    }
    ...
    return entries
      .map(m => {
        ...
        const npv  = band
          ...
          : { value: calc.npv.value, low: calc.npv.value, high: calc.npv.value };
        return {
          label:               m[labelField],
  ...
  function renderRowChart(container, rows, colHeaderLabel, showMeasureName = false) {
    ...
    rows.forEach((row, i) => {
      navRows.push({ measureId: row.measureId, label: row.label, measureName: row.measureName, baselineName: row.baselineName });
      ...
      const co2Col = co2Pos ? COLOR_FAVORABLE : co2Neg ? COLOR_COSTLY : CLR_SUB;
      ...
      if (co2Neg) {
        svg.append('text')
          ...
          .attr('fill', COLOR_COSTLY).text('zvyšuje emise');
      }
      ...
      const fuelCol = fuelTot == null || fuelTot === 0 ? CLR_SUB : fuelTot > 0 ? COLOR_FAVORABLE : COLOR_COSTLY;
  ...
  let navRows       = [];     // every clickable row in visual order — drives ←/→ nav in the detail card
  ...
  // While the detail card is open: Esc closes it, ←/→ loop through all measures.
  function detailKeydown(e) {
    ...
    if (e.key === 'Escape') {
      ...
      e.preventDefault();
    } else if ((e.key === 'ArrowRight' || e.key === 'ArrowLeft') && navRows.length) {
      const i = navRows.findIndex(r => r.measureId === openMeasureId);
      if (i < 0) return;
      const step = e.key === 'ArrowRight' ? 1 : -1;
      const n = navRows[(i + step + navRows.length) % navRows.length];
      openMeasureId = n.measureId;
      openRowMeta   = { label: n.label, measureName: n.measureName, baselineName: n.baselineName };
      renderOpenDetail();
      e.preventDefault();
    }
  }
  ...
  function lifetimeEnergyCost(m) {
    ...
    const isFve = !m.transport_category && (m.electricity_savings > 0);
    const key = isFve ? 'electricity' : m.fuel.toLowerCase();
    const consumption = m.transport_category
      ? (m.demand_energy_per_100km || 0) * (m.mileage || 0) / 100
      : isFve
        ? (m.demand_electricity_mwh || 0) * (1 - (m.electricity_savings || 0))
        : (m.demand_heat_measure_mwh || 0);
    const dr = (state.discountRate || 0) / 100;
    let total = 0;
    for (let y = 1; y <= life; y++) {
      ...
      const discFactor = dr === 0 ? 1 : 1 / Math.pow(1 + dr, y);
      total += consumption * price * discFactor;
    }
    return total;
  }
  ...
  function makeLifeEnergyCostFn(result) {
    if (!result || !(result.yearByYear || []).length) return x => lifeEnergy(x);
    const dr   = (state.discountRate || 0) / 100;
    const rows = result.yearByYear.filter(r => r.year > 0);
    const discountedOpex = (opexKey, maintFields) => x => {
      const maint = maintFields.reduce((s, f) => s + (x[f] || 0), 0);
      return czkPlain(rows.reduce((sum, r) => {
        const discFactor = dr === 0 ? 1 : 1 / Math.pow(1 + dr, r.year);
        return sum + (r[opexKey] - maint) * discFactor;
      }, 0));
    };
    const maintFields = result.measure.transport_category
      ? ['opex_maintenance_czk', 'opex_repairs_czk', 'opex_insurance_czk']
      : ['opex_maintenance_czk'];
    return x => x === result.measure
      ? discountedOpex('opexMeasure', maintFields)(x)
      : discountedOpex('opexBaseline', maintFields)(x);
  }

  function comparisonSpecs(m, result) {
    const lifeEnergyCost = makeLifeEnergyCostFn(result);
    if (m.transport_category) {
      return [
        ['Pořizovací cena',            x => czkPlain(capexTotal(x))],
        ['Náklady za energie za životnost', lifeEnergyCost],
        ['Provoz za životnost',        x => { const dr = (state.discountRate || 0) / 100; const ann = (x.opex_maintenance_czk || 0) + (x.opex_repairs_czk || 0) + (x.opex_insurance_czk || 0); const life = measureLifetime(x); let s = 0; for (let y = 1; y <= life; y++) s += ann * (dr === 0 ? 1 : 1 / Math.pow(1 + dr, y)); return czkPlain(s); }],
        ['Spotřeba',                   x => x.demand_energy_per_100km != null
          ? (x.fuel === 'Electricity'
              ? fmtInt.format(Math.round(x.demand_energy_per_100km * 1000)) + ' kWh/100 km'
              : fmtInt.format(Math.round(x.demand_energy_per_100km * 10) / 10) + ' l/100 km')
          : '—'],
    ...
    return [
      ...
      [m.electricity_savings > 0 ? 'Náklady za elektřinu za životnost' : 'Náklady za vytápění za životnost', m.electricity_savings > 0 ? (() => {
        // FVE: show electricity costs only (gas heating is identical for both sides)
        const scen = (data.fuel_scenarios || []).find(s => s.scenario === state.fuelScenario);
        const life = measureLifetime(m);
        const dr   = (state.discountRate || 0) / 100;
        return x => {
          if (!scen || !scen.prices || !scen.prices.length || !life) return '—';
          const cons = (x.demand_electricity_mwh || 0) * (1 - (x.electricity_savings || 0));
          let total = 0;
          for (let y = 1; y <= life; y++) {
            let pe = scen.prices[0];
            for (const p of scen.prices) if (p.year_investment <= y) pe = p;
            if (pe.electricity == null) return '—';
            const discFactor = dr === 0 ? 1 : 1 / Math.pow(1 + dr, y);
            total += cons * pe.electricity * discFactor;
          }
          return czkPlain(total);
        };
      })() : lifeEnergyCost],
      ['Údržba za životnost',           x => { if (x.opex_maintenance_czk == null) return '—'; const dr = (state.discountRate || 0) / 100; const life = measureLifetime(m); let s = 0; for (let y = 1; y <= life; y++) s += x.opex_maintenance_czk * (dr === 0 ? 1 : 1 / Math.pow(1 + dr, y)); return czkPlain(s); }],
      [m.electricity_savings > 0 ? 'Spotřeba elektřiny' : 'Spotřeba paliva', m.electricity_savings > 0
        ? x => { const cons = (x.demand_electricity_mwh || 0) * (1 - (x.electricity_savings || 0)); return fmtInt.format(Math.round(cons * 10) / 10) + ' MWh/rok'; }
        : x => x.demand_heat_measure_mwh != null ? x.demand_heat_measure_mwh + ' MWh/rok' : '—'],
      ...(m.electricity_savings > 0 ? [] : [['Účinnost', x => x.efficiency != null ? String(x.efficiency).replace('.', ',') : '—']]),
      ['Životnost',                     x => (m.electricity_savings > 0 && x !== m) ? '—' : x.lifetime != null ? Math.round(x.lifetime) + ' let' : '—'],
    ];
  }

  // Measure vs. baseline parameters — parameters as columns, one row per variant.
  function renderComparison(container, measure, baseline, result) {
    if (!measure) return;
    const specs = comparisonSpecs(measure, result);
  ...
  function renderDetailPanel(row, result) {
    ...
    // Anchor the card just below the sticky navbar so it never covers the controls.
    const nav = document.getElementById('secondary-navbar');
    const navBottom = nav ? nav.getBoundingClientRect().bottom : 0;
    bar.style.setProperty('--cb-detail-top', Math.max(navBottom + 12, 12) + 'px');
    ...
    const dovozNote = s2
      ? (s2 < 0
          ? 'Včetně zvýšení dovozu plynu (<strong>' + fmtMWh(Math.abs(s2)) + '</strong>) na výrobu elelektřiny.'
          : 'Včetně snížení dovozu plynu (<strong>' + fmtMWh(s2) + '</strong>) na výrobu elektřiny.')
      : null;
    ...
    const sens = buildSensRows(measure, result.sensitivity, result);
    if (sens.length) {
      const wrap = document.createElement('div');
      wrap.className = 'rd-chart--sens';
      const lbl = document.createElement('div');
      lbl.className = 'row-detail-section-label';
      lbl.textContent = 'Co ovlivňuje Net Present Value';
      ...
      inner.appendChild(wrap);
      renderSensTable(tornEl, sens, result.npv);
    }
    ...
    if (measure) {
      ...
      inner.appendChild(wrap);
      renderComparison(cmp, measure, baseline, result);
    }
  }
  ...
  function buildSensRows(measure, rawSens, result) {
    const carbon      = measure ? carbonSensitivity(measure) : null;
    // Skip electricitySensitivity for transport — the built-in already covers it via fuelSet
    const alreadyHasElec = (rawSens || []).some(s => s.param === 'Cena elektřiny');
    const electricity = (measure && !alreadyHasElec) ? electricitySensitivity(measure) : null;
    const capexLbl = l => (l && l.includes('+')) ? 'pesimistická' : 'optimistická';
    return [...(rawSens || []), ...(carbon ? [carbon] : []), ...(electricity ? [electricity] : [])].flatMap(s => {
  ...
  function electricitySensitivity(measure) {
    const base = calcOpts(measure);
    const delta = 0.1;  // ±10 % matching SENSITIVITY_THRESHOLDS.Electricity
    const npvAt = mult => { try { return CostsBenefits.calculate({ ...base, fuelPriceName: 'Electricity', fuelPriceMult: mult }).npv; } catch (_) { return null; } };
    const a = npvAt(1 - delta), b = npvAt(1 + delta);
    if (a == null || b == null) return null;
    if (a === b) return null;   // electricity price has no effect → omit the row
    return {
      param: 'Cena elektřiny',
      minNpv: Math.min(a, b), maxNpv: Math.max(a, b),
      minLabel: a <= b ? '−10 %' : '+10 %',
      maxLabel: a <= b ? '+10 %' : '−10 %',
    };
  ...
  function renderSensTable(container, sensInput, baseNpv) {
    ...
    sens.forEach((s, i) => {
      ...
      const npvCol = v => v >= 0 ? COLOR_FAVORABLE : COLOR_COSTLY;
      const zx = plotX + x(0);
  ...
  function renderAll() {
    ...
    navRows = [];   // repopulated (in visual order) as the measure charts render below
  ...
  function init() {
    ...
    document.addEventListener('keydown', detailKeydown);
  }
