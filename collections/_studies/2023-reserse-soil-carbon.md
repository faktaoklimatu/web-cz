...
sections:
  - # ID sekce (pro hypertextové odkazy).
    id: sekvestrace
    # Nadpis sekce.
    title: Ukládání uhlíku do půdy a jeho vliv na mitigaci klimatické změny
    # Úvodní odstavec sekce.
    description: V následujících studiích lze najít základní informace o ukládání (sekvestraci) uhlíku do půdy a přehled metod, které je možné k ukládání využívat.
    # Seznam studií v sekci.
    studies:
      - name: Carbon sequestration in soil. Lal et al., 2015
        link: https://www.researchgate.net/profile/Wakene-Negassa/publication/283457192_Carbon_sequestration_in_soil/links/5faef9f292851cf24ccdfe37/Carbon-sequestration-in-soil.pdf
      ...
      - name: Role of soil carbon in natural climate solutions. Bossio et al., 2020
        ...
        description: |
          * Analýza tzv. dvanácti opatření blízkých přírodě (Natural Climate Solutions – NCS), podporujících ukládání a uchovávání uhlíku v půdě – pro každé opatření je vyčíslen jeho mitigační potenciál v roce 2030 (v GtCO<sub>2</sub>eq ročně), ve třech různých úrovních finanční náročnosti
  ...
  - id: faktory
    title: Faktory ovlivňující množství uhlíku v půdě
    description: Tyto studie se zabývají množstvím uhlíku v půdě a faktory, které mohou toto množství ovlivňovat (např. typ půdy, množství srážek, klimatické podmínky či způsob hospodaření).
    studies:
  ...
  - id: oteplovani
    title: Vliv globálního oteplování na množství uhlíku v půdě
    description: Při modelování scénářů nebo výpočtu sekvestračního potenciálu ekosystémů je důležité brát v úvahu různé související procesy, např. pozitivní zpětnou vazbu mezi globálním oteplováním a dynamikou půdního uhlíku. Tomuto výzkumu se věnují následující studie.
    studies:
  ...
  - id: kapacita
    title: Schopnost půdy vázat uhlík a vhodné postupy sekvestrace
    description: Tyto studie se zaměřují na schopnost různých typů půd uchovávat uhlík. Tato schopnost není neomezená a ovlivňuje ji řada faktorů.
    studies:
      ...
      - name: Soil carbon storage informed by particulate and mineral-associated organic matter. Cotrufo at al., 2019
        link: https://par.nsf.gov/servlets/purl/10147876
        source: Nature Geoscience
        description: |
          * Výzkum zaměřený na stav půdy na evropských pastvinách a v lesích
          * Pastviny: v půdní organické hmotě (SOM) dominuje typ uhlíku vázaný na minerály MAOM-C (dělení podle [Lavallee et al, 2019](https://onlinelibrary.wiley.com/doi/full/10.1111/gcb.14859?casa_token=Gv_aZvNojLsAAAAA%3Ahow5n_aE0pdON2xUSHCoCIGAqjhr2D5d3TgBbmTZF5dQMjD24A55Yvg0xA1GXGBuuKx1uHhy9jwS_SY)), který v půdě zůstává dlouho, jeho sekvestrace je ale náročná na množství dusíku a půda může dosáhnout své maximální sekvestrační kapacity
          * Lesy: schopné sekvestrovat více uhlíku, ten je zde však uložen v méně trvalé formě pevných organických částic (POM)
  - id: mereni
    title: Měření množství uhlíku v půdě
    description: Tyto studie poskytují přehled různých metod, které se využívají k měření množství uhlíku v půdě. Přesnost a opakovatelnost měření je klíčová například při posuzování efektivity offsetových projektů.
    studies:
  ...
  - id: organicky
    title: Organický uhlík v půdě
    description: Studie představuje dva hlavní typy organického uhlíku v půdě – toto rozdělení je důležité pro pochopení dynamiky půdních procesů a aplikaci vhodných metod sekvestrace.
    studies:
  ...
  - id: anorganicky
    title: Anorganický uhlík v půdě
    description: Tyto studie představují základní úvod do tématu anorganického uhlíku v půdě (*soil inorganic carbon*, SIC), kterému je věnována mnohem menší pozornost než výzkumu organického uhlíku v půdě (*soil organic carbon*, SOC). Světové zásoby SIC jsou menší než zásoby SOC, jsou však považovány za stabilnější a studie ukazují významný vliv anorganického půdního uhlíku na globální uhlíkový cyklus.
    studies:
  ...
  - id: iniciativa-4p1000
    title: Iniciativa 4 per mille Soils for Food Security and Climate
    description: Tyto studie poskytují základní informace o iniciativě 4 per mille a zkoumají reálné využití jejích návrhů v praxi.
    studies:
...
{% comment %}
  Verze rešerše pro menší displeje: seznam studií lineárně za sebou.
{% endcomment %}
<dl class="d-md-none">
  {% for study in section.studies %}
  <dt class="mt-3"><a href="{{ study.link }}">{{ study.name }}</a></dt>
  <dd>
    <p class="mb-1"><cite>{{ study.source }}</cite></p>

    {{ study.description | markdownify }}
  </dd>
  {% endfor %}
</dl>

{% comment %}
  Plná verze rešerše: tabulka studií se třemi sloupci.
{% endcomment %}
<table class="table table-striped table-hover d-none d-md-table mt-4 mb-4">
  <thead>
    <tr>
      <th scope="col" class="text-uppercase">Studie/článek</th>
      <th scope="col" class="text-uppercase">Zdroj</th>
      <th scope="col" class="text-uppercase" width="50%">Shrnutí</th>
    </tr>
  </thead>
  <tbody>
    {% for study in section.studies %}
    <tr>
      <td class="align-middle font-weight-bold">
        <a href="{{ study.link }}">{{ study.name }}</a>
      </td>
      <td class="align-middle"><cite>{{ study.source }}</cite></td>
      <td class="align-middle">{{ study.description | markdownify }}</td>
    </tr>
    {% endfor %}
  </tbody>
</table>
