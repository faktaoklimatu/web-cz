---
layout:      survey
title:       "Klimatické zákony států EU"
slug:        2023-reserse-klimaticke-zakony
weight:      303
published:   2023-09-12
tags-scopes: [ eu ]
tags-topics: [ opatreni ]
use-preview-image: true
caption:     "Přehled základních informací o klimatických zákonech ve státech EU."
intro: |
  Již šestnáct států Evropské unie přijalo klimatický zákon, ve kterém v různé míře detailu předkládají své ambice v oblasti snižovování emisí skleníkových plynů a cesty, které k tomu povedou. Zasazení klimatických závazků do zákona ukládá aktérům ve veřejné i soukromé sféře větší odpovědnost k jejich plnění a zároveň činí klimatické ambice odolnějšími vůči změnám politické nebo ekonomické situace v zemi. Česko zatím svůj klimatický zákon nemá.
items:
  - code: BGR
    name: Bulharsko
    flag-url: https://upload.wikimedia.org/wikipedia/commons/9/9a/Flag_of_Bulgaria.svg
    law-year: 2022
    url: https://lex.bg/laws/ldoc/2136124027
    targets:
    budgets: false
    plans: false
    reporting: false
    advisory: false
    public: false
    notes: Zákon se zaměřuje především na implementaci unijní legislativy a plnění závazků Bulharska v rámci EU.
  - code: DNK
    name: Dánsko
    flag-url: https://upload.wikimedia.org/wikipedia/commons/9/9c/Flag_of_Denmark.svg
    law-year: 2021
    url: https://www.retsinformation.dk/eli/lta/2021/2580
    targets: |
      2025: <b>−50–54 %</b><br>
      2030: <b>−70 %</b><br>
      <b>2050: klimatická neutralita</b>
    budgets: true
    plans: true
    reporting: true
    advisory: true
    public: true
    notes: Dánsko v zákonu vyjadřuje ambici stát se globálním lídrem v úsilí o ochranu klimatu.
  - code: FIN
    name: Finsko
    flag-url: https://upload.wikimedia.org/wikipedia/commons/b/bc/Flag_of_Finland.svg
    law-year: 2022 
    url: https://www.finlex.fi/en/laki/kaannokset/2015/en20150609_20220423.pdf
    targets: |
      2030: <b>−60 %</b><br>
      <b>2035: klimatická neutralita</b><br>
      2040: <b>−80 %</b><br>
      2050: <b>−95 %</b>
    budgets: false
    plans: true
    reporting: true
    advisory: true
    public: true
    notes: Ačkoliv Finsko zamýšlí dosáhnout klimatické neutrality již v roce 2035 (díky pohlcování emisí lesy), emise, které vypouští do atmosféry se zavázalo snižovat i poté a stát se tak klimaticky negativním státem.
  - code: FRA
    name: Francie
    flag-url: https://upload.wikimedia.org/wikipedia/commons/c/c3/Flag_of_France.svg
    law-year: 2021
    url: https://www.legifrance.gouv.fr/jorf/id/JORFTEXT000043956924
    targets: |
      2030: <b>−40 %</b><br>
      <b>2050: klimatická neutralita</b>
    budgets: true
    plans: true
    reporting: true
    advisory: true
    public: true
    notes: Zákon obsahuje i konkrétní opatření pro ochranu klimatu v různých odvětvích (školství, dopravě, zemědělství atd.).
  - code: HRV
    name: Chorvatsko
    flag-url: https://upload.wikimedia.org/wikipedia/commons/1/1b/Flag_of_Croatia.svg
    law-year: 2019
    url: https://www.zakon.hr/z/2435/Zakon-o-klimatskim-promjenama-i-za%C5%A1titi-ozonskog-sloja
    targets:
    budgets: false
    plans: true
    reporting: false
    advisory: false
    public: false
    notes: Zákon se zaměřuje především na implementaci unijní legislativy a plnění závazků Chorvatska v rámci EU.
  - code: IRL
    name: Irsko
    flag-url: https://upload.wikimedia.org/wikipedia/commons/4/45/Flag_of_Ireland.svg
    law-year: 2021
    url: https://assets.gov.ie/127957/ab70a65d-68c1-4947-983b-babf920cc4dc.pdf
    targets: |
      2030: <b>−51 %</b><br>
      <b>2050: klimatická neutralita</b>
    reference-year: 2018
    budgets: true
    plans: true
    reporting: true
    advisory: true
    public: true
    notes: Ministři a ministryně jsou odpovědní za plnění cílů a přijímání opatření v resortu, který vedou. O pokroku pak každoročně reportují.
  - code: LTU
    name: Litva
    flag-url: https://upload.wikimedia.org/wikipedia/commons/1/11/Flag_of_Lithuania.svg
    law-year: 2021
    url: https://www.e-tar.lt/portal/legalAct.html?documentId=96e945d04df111ec862fdcbc8b3e3e05
    targets:
    budgets: false
    plans: true
    reporting: false
    advisory: true
    public: false
    notes: Spíše než o rámec pro klimatickou politiku, jde o zákon, který se věnuje především obchodování s emisními povolenkami a dalším finančním nástrojům. 
  - code: LUX
    name: Lucembursko
    flag-url: https://upload.wikimedia.org/wikipedia/commons/d/da/Flag_of_Luxembourg.svg
    law-year: 2020
    url: https://data.legilux.public.lu/filestore/eli/etat/leg/loi/2020/12/15/a994/jo/fr/html/eli-etat-leg-loi-2020-12-15-a994-jo-fr-html.html
    targets: |
      2030: <b>−55 %</b><br>
      <b>2050: klimatická neutralita</b>
    reference-year: 2005
    budgets: false
    plans: true
    reporting: true
    advisory: true
    public: true
    notes: Vytváří platformu pro klima, která zprostředkovává dialog mezi představitely komun, organizacemi občanské společnosti, vedoucími firem, investory a širokou veřejností.
  - code: HUN
    name: Maďarsko
    flag-url: https://upload.wikimedia.org/wikipedia/commons/c/c1/Flag_of_Hungary.svg
    law-year: 2020
    url: https://njt.hu/jogszabaly/2020-44-00-00
    targets: |
      2030: <b>−40 %</b><br>
      <b>2050: klimatická neutralita</b>
    budgets: false
    plans: false
    reporting: false
    advisory: false
    public: false
    notes: Udává pouze cíle pro snižování emisí skleníkových plynů a základní principy.
  - code: MLT
    name: Malta
    flag-url: https://upload.wikimedia.org/wikipedia/commons/7/73/Flag_of_Malta.svg
    url: https://legislation.mt/eli/cap/543/eng/pdf
    law-year: 2015
    targets:
    budgets: false
    plans: true
    reporting: false
    advisory: false
    public: true
    notes: Implementaci zákona by měl dohlížet _Climate Action Board_, který sdružuje premiéra, ministry a další aktéry z řad odborníků a odbornic.
  - code: DEU
    name: Německo
    flag-url: https://upload.wikimedia.org/wikipedia/commons/b/ba/Flag_of_Germany.svg
    law-year: 2021
    url: http://www.gesetze-im-internet.de/ksg/KSG.pdf
    targets: |
      2030: <b>−65 %</b><br>
      2040: <b>−88 %</b><br>
      <b>2045: klimatická neutralita</b>
    budgets: true
    plans: true
    reporting: true
    advisory: true
    public: true
    notes: Neuvádí nutnost vytvářet krátkodobé akční plány a dlouhodobé strategie.
  - code: NLD
    name: Nizozemsko
    flag-url: https://upload.wikimedia.org/wikipedia/commons/2/20/Flag_of_the_Netherlands.svg
    law-year: 2019
    url: http://www.ilo.org/dyn/natlex/docs/ELECTRONIC/108819/134700/F-73875454/NLD108819.pdf
    targets: |
      2030: <b>−49 %</b><br>
      2050: <b>−95 %</b>
    budgets: false
    plans: true
    reporting: true
    advisory: false
    public: false
    notes: Roli odborného porádního orgánu přebírá již existující vládní agentura.
  - code: PRT
    name: Portugalsko
    flag-url: https://upload.wikimedia.org/wikipedia/commons/5/5c/Flag_of_Portugal.svg
    law-year: 2021
    url: https://files.dre.pt/1s/2021/12/25300/0000500032.pdf
    targets: |
      2030: <b>−55 %</b><br>
      2040: <b>−65 %</b><br>
      <b>2050: klimatická neutralita</b>
    reference-year: 2005
    budgets: true
    plans: true
    reporting: true
    advisory: true
    public: true
  - code: ESP
    name: Španělsko
    flag-url: https://upload.wikimedia.org/wikipedia/commons/9/9a/Flag_of_Spain.svg
    law-year: 2021
    url: https://www.boe.es/boe/dias/2021/05/21/pdfs/BOE-A-2021-8447.pdf
    targets: |
      2030: <b>−23 %</b><br>
      <b>2050: klimatická neutralita</b>
    budgets: false
    plans: true
    reporting: true
    advisory: true
    public: true
    notes: Vyžaduje, aby firmy připravily vlastní klimatické plány.
  - code: SWE
    name: Švédsko
    flag-url: https://upload.wikimedia.org/wikipedia/commons/4/4c/Flag_of_Sweden.svg
    law-year: 2017
    url: https://www.government.se/articles/2021/03/swedens-climate-policy-framework/
    targets: |
      2030: sektorové cíle<br>
      2040: sektorové cíle<br>
      <b>2045: klimatická neutralita</b>
    budgets: false
    plans: true
    reporting: true
    advisory: true
    public: true
    notes: |
      Klimatická politika Švédska je tvořena třemi pilířy: klimatickým zákonem, cíli pro snižování emisí skleníkových plynů a radou pro klimatickou politiku.
  - code: GRC
    name: Řecko
    flag-url: https://upload.wikimedia.org/wikipedia/commons/5/5c/Flag_of_Greece.svg
    law-year: 2022
    url: https://www.elinyae.gr/sites/default/files/2022-05/105a_2022.pdf
    targets: |
      2030: <b>−55 %</b><br>
      2040: <b>−80 %</b><br>
      <b>2050: klimatická neutralita</b>
    budgets: true
    plans: false
    reporting: true
    advisory: true
    public: true
---
{:.narrow-text}
Rešerše ukazuje, jaké aspekty jednotlivé klimatické zákony (ne)obsahují. Hodnotí, zda zákon obsahuje: cíle pro snižování emisí skleníkových plynů, uhlíkové rozpočty, průběžné plánování kroků k transformaci a reportování o pokroku, zřízení poradních orgánů nezávislých na vládě a účast veřejnosti při rozhodování o podobě klimatické politiky státu.

{:.narrow-text}
K roku 2023 přijalo klimatických zákon 16 z 27 států Unie. Dále probíhá diskuze o klimatickém zákoně na Slovensku nebo ve Slovinsku.

<ul class="inline-bullet-list">
    {%- for country in page.items %}
    <li><a href="#{{ country.code }}">{{ country.name }}</a></li>
    {%- endfor %}
</ul>

<div class="climate-laws-survey-countries narrow-text">
    {% for country in page.items %}
    {% assign reference-year = country.reference-year | default: 1990 %}
    {% include includes-local/klimaticke-zakony/country.html
        code=country.code
        name=country.name
        flag-url=country.flag-url
        law-year=country.law-year
        url=country.url
        targets=country.targets
        reference-year=reference-year
        budgets=country.budgets
        plans=country.plans
        reporting=country.reporting
        advisory=country.advisory
        public=country.public
        notes=country.notes
    %}
    {% endfor %}
</div>
